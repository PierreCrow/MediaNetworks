...
public class CameraSourcePreview extends ViewGroup {
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        int var10000 = (int)((float)layoutWidth / (float)width * (float)height);

        for(int i = 0; i < this.getChildCount(); ++i) {
            this.getChildAt(i).layout(0, 0, childWidth, layoutHeight);
        }

        try {
            this.startIfReady();
        } catch (SecurityException var13) {
            Log.e("CameraSourcePreview", "Do not have permission to start the camera", var13);
        } catch (IOException var14) {
            Log.e("CameraSourcePreview", "Could not start camera source.", var14);
        }
