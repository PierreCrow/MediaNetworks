...
public class mt_WorkOder_Actions_Durante_Tab extends Fragment
    ...
    void initAll(View x)
    {
        ...
        ArrayList<LineaTiempo> lineaTiemposDurante=new ArrayList<>();


        for(LineaTiempo lineaTiempo:mt_WorkOrder_Actions_Tabs.lineaTiempos)
        {

            if(lineaTiempo.getCondicion()== Constantes.CONDICION_LINEA_TIEMPO.DURANTE)
            {
                lineaTiemposDurante.add(lineaTiempo);
            }
        }

        adapter = new mt_WorkOrder_Action_AdapterList(listener, getContext(), lineaTiemposDurante);
